...
class Labelme2YOLO(object):
    ...
    def _save_yolo_image(self, json_data, json_name, image_dir_path, target_dir):
        ...
        img_type = json_data['imagePath'].split('.')[1]
        img_name = json_name.replace('.json', '.' + img_type)

        img_path_org = os.path.join(args.json_dir, img_name)
        img_path = os.path.join(image_dir_path, target_dir,img_name)

        if not os.path.exists(img_path):
            shutil.copyfile(img_path_org, img_path)
...
if __name__ == '__main__':
    ...
    print(args.seg)
    #args.json_dir = r"C:\Share\Data_Model\2obj\test"
